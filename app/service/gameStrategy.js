...
class GameStrategyService extends Service {
  constructor(ctx) {
    super(ctx);
  }
  ...
  async index(payload) {
    ...
    try {
      let result = {};
      let fields =
        "game_strategy.id, game_strategy.strategy_title, game_strategy.strategy_thumbnail, game_strategy.platform, game_strategy.create_date, game_strategy.game_id, game.game_name";
      let condition = "1 = 1";
      ...
      if (payload.platform) {
        condition += ` AND game_strategy.platform LIKE '%${payload.platform}%'`;
      }

      if (payload.startDate && payload.endDate) {
        condition += ` AND game_strategy.create_date BETWEEN '${
          payload.startDate
        }' AND '${payload.endDate}'`;
      }
      ...
      let sql = `select ${fields} from game_strategy LEFT JOIN game ON game_strategy.game_id = game.id where ${condition} ORDER BY ${orderBy} LIMIT ${limit} OFFSET ${offset}`;

      let countSql = `select count(*) as total from game_strategy where ${condition}`;
      //查询结果的数组
  ...
  async destroy(ids) {
    try {
      const result = await Promise.all(
        ids.map(async id => {
          //删除数据
          let singleResult = await this.app.mysql.delete("game_strategy", {
            id: id
          });
...
module.exports = GameStrategyService;
