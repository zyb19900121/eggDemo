...
class GameGalleryController extends Controller {
  async index() {
  ...
  async create() {
    const { ctx } = this;
    let payload = {};
    payload.gameId = ctx.request.body.gameId;
    payload.imageList = ctx.request.body.imageList;
    try {
      const result = await ctx.service.gameGallery.create(payload);
      ctx.body = result;
      ctx.status = 200;
    } catch (error) {
      ctx.body = { error: error.toString() };
  ...
  async destroy() {
    ...
    // ctx.validate(createRule, ctx.request.body);
    try {
      const result = await service.gameGallery.destroy(id);
      // 设置响应体和状态码
      ctx.body = result;
      ctx.status = 200;
    } catch (error) {
...
module.exports = GameGalleryController;
